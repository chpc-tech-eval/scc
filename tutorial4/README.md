...
1. [Automating the Deployment of Instances Using Terraform](#automating-the-deployment-of-instances-using-terraform)
    ...
    1. [Automating the Deployment of your OCI Instances Using Terraform](#automating-the-deployment-of-your-oci-instances-using-terraform)
        1. [Install and Initialize Terraform](#install-and-initialize-terraform-2)
        1. [Configure OCI API Credentials](#configure-oci-api-credentials)
        1. [Create `providers.tf` and `main.tf` Files](#create-providerstf-and-maintf-files-1)
        1. [Generate, Deploy and Apply Terraform Plan](#generate-deploy-and-apply-terraform-plan-2)
...
## Automating the Deployment of your OCI Instances Using Terraform

Terraform is an open-source tool that allows you to define and manage your cloud infrastructure as code (IaC).
Using Terraform, you can reproducibly deploy and configure cloud resources such as compute nodes on Oracle Cloud Infrastructure (OCI).

In this section, you will deploy an **additional compute node** on **OCI** using Terraform.
...
If you have not already installed Terraform on your **head node** (refer to the previous sections), do so now.

Once installed, verify that Terraform is available:
...
Unlike AWS (which uses access keys) or OpenStack (which uses `clouds.yaml`), OCI requires an **API Signing Key** and a specific config file.

1.  **Generate API Key:**
    * Log in to the OCI Console.
    * Click on the **Profile** icon (top right) &rarr; **My Profile**.
    * Navigate to **API Keys** (bottom left menu) &rarr; **Add API Key**.
    * Select **Generate API Key Pair**.
    * Download the **Private Key** and save it to `~/.oci/oci_api_key.pem` on your head node.
    * Click **Add**.

    <p align="center"><img alt="OCI API Key Generation" src="./resources/oci_api_key.png" width=900 /></p>

2.  **Create Config File:**
    After clicking "Add", OCI will display a configuration snippet in a text box.
    
    Create the config file on your head node:
    ...
    Paste the configuration provided by the OCI console into this file. It should look similar to this:
    ...
    > [!NOTE]
    > **Important:** You must manually edit the `key_file` line in the pasted config to point to where you actually saved the key.
    > Example: `key_file=/home/ubuntu/.oci/oci_api_key.pem`

    Secure the files (Terraform will reject them if they are too open):
    ```bash
...
2.  **Create the Provider Configuration (`providers.tf`)**

    This file tells Terraform to use the Oracle Cloud provider.
...
3.  **Create the Instance Definition (`main.tf`)**

    Define your OCI compute node. You will need to gather your Compartment OCID, Subnet OCID, and Image OCID from the OCI Console.
    ...
    resource "oci_core_instance" "terraform_demo_instance" {
      # Use the first Availability Domain
      availability_domain = data.oci_identity_availability_domains.ads.availability_domains[0].name
      compartment_id      = "<YOUR_COMPARTMENT_OCID>"
      shape               = "VM.Standard.E2.1.Micro"

      display_name        = "scc25-oci-cn-experimental"
      ...
      metadata = {
        # Path to your public SSH key on the head node
    ...
    > [!TIP]
    > **Finding OCIDs:** You can find the OCID for your Image, Subnet, and Compartment by viewing those resources in the OCI web console and clicking "Copy" next to the OCID field.
...
2.  **Generate a Terraform Plan**
    ...
    ```bash
    terraform plan -out ~/terraform_oci/oci-plan
    ```
...
3.  **Apply the Terraform Plan**
    ...
    ```bash
    terraform apply ~/terraform_oci/oci-plan
    ```

#### Verify New Instance in OCI

Once the deployment completes, confirm that your instance is active:

1. Open the **OCI Console**.
2. Navigate to **Compute** &rarr; **Instances**.
3. Verify the instance named `scc25-oci-cn-experimental` is in the **Running** state.
4. Connect via SSH using the public IP provided in the dashboard.

    ```bash
    ssh opc@<instance-public-ip>
    ```
    *(Note: Use `ubuntu` user if you selected an Ubuntu image)*

> [!CAUTION]
> **Clean Up Resources**
> OCI has strict limits on the number of Always Free instances you can run. To avoid "Out of Capacity" or "Limit Exceeded" errors when deploying your actual cluster later, **destroy this test environment when finished**:
> ```bash
> terraform destroy
> ```
